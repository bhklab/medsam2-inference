...
from utils import dice_multi_class, resize_grayscale_to_rgb_and_resize, mask3D_to_bbox, preprocess, AddedPathLength, overlay_bbox
...
class MedSAM3DInferenceConfig(BaseModel):
    ...
    overlay_bbox: bool = Field(default=False, description="Whether to overlay the bounding box on the image.")
    ...
    def run(self):
        ...
        for _, row in tqdm(dataset.iterrows(), total=len(dataset), desc="Processing patients"):
            ...
            try:
                ...
                if self.config.overlay_bbox:
                    overlay_bbox(image_array, bbox2d, z_mid_orig, bbox_dir / f'{patient_id}_bbox.png')
...
if __name__ == '__main__':
    inference()
#            dataset_csv = "data/procdata/RADCURE_GTVp/metadata/mit_RADCURE_GTVp_medsam_input_apptainer.csv",
#            output_dir = "data/results/RADCURE_GTVp",
#            overlay_bbox = True
#            )
